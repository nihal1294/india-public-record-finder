...
export function EvidenceStep({ candidate, source, copy }: { candidate: SearchCandidate; source: string; copy: Messages['evidence'] }) {
  ...
  const sourceReference = [
    candidate.source_part && formatMessage(copy.part, { sourcePart: candidate.source_part }),
    candidate.source_page !== undefined && formatMessage(copy.page, { sourcePage: candidate.source_page }),
  ].filter(Boolean).join(' · ')
  return (
    <section className="evidence-step" aria-live="polite">
      <div className="evidence-heading"><h2 id="evidence-heading" tabIndex={-1}>{copy.heading}</h2>{sourceReference && <p>{sourceReference}</p>}</div>
      ...
      {showLarger && <ModalDialog title={copy.cropLabel} closeLabel={copy.close} onClose={() => setShowLarger(false)}>{sourceReference && <p className="modal-source-reference">{sourceReference}</p>}<img alt={copy.cropLabel} className="evidence-image evidence-image-large" src={source} /></ModalDialog>}
    </section>
  )
}
